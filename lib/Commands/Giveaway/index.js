            ...
            if (!isGroupMsg) {
                ...
                if (Date.now() >= timeouts[chatId].cooldown) {
                    ...
                    if (argl[0] === '-buy') {
                        ...
                        const insertUser = {
                            tickets: /[0-9]+/gi.test(argl[1]) ? Number(argl[1]) : 1,
                        };
                        ...
                        if (Object.keys(giveaways[chatId]).includes(user)) {
                            ...
                            ticketsCoins += giveaways[chatId][user].tickets;
                        }
                        ...
                        if (leveling.coin >= ticketsCoins) {
                            ...
                            if (Object.keys(giveaways[chatId]).length >= envInfo.parameters.lotteryUsers.value) {
                                ...
                                if (timeouts[chatId].time === 0 && timeouts[chatId].clock === '') {
                                    ...
                                    timeouts[chatId].clock = setTimeout(async () => {
                                        ...
                                        envInfo.results.value = await kill.sendMessage(chatId, {
                                            text: language(region, 'Typings', 'Winner', true, true, {
                                ...
                                envInfo.results.value = await kill.sendMessage(chatId, {
                                    text: language(region, 'Typings', 'Start', true, true, {
                                ...
                                envInfo.results.value = await kill.sendMessage(chatId, {
                                    text: language(region, 'Typings', 'Buy', true, true, {
                                        chance: winChances(giveaways[chatId], user),
                                        amount: giveaways[chatId][user].tickets,
                                        item: 'Tickets',
                                        win: giveaways.prize,
                        ...
                        envInfo.results.value = await kill.sendMessage(chatId, { text: language(region, 'Typings', 'NoGames', true, true, envInfo) }, { quoted: quoteThis });
