...
async function searchYouTube(
    ...
    quoteThis = envInfo.functions.search.arguments.quoteThis.value,
) {
...
async function createDownload(
    ...
    quoteThis = envInfo.functions.down.arguments.quoteThis.value,
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if (arks.includes('--help-dev') && isOwner === true) {
                ...
                if (haveURL.isURL === false) {
                    /* Pesquisa pelo video */
                    youTubeData = await searchYouTube(body, kill, chatId, quoteThis);
                }
                ...
                if ((youTubeData.value.length !== 0 && youTubeData.success === true && Array.isArray(youTubeData.value)) || haveURL.isURL === true) {
                    ...
                    /* Inicia o download */
                    const baixarYouTube = await createDownload(youTubeData.value.url, downFormat, kill, chatId, quoteThis, haveURL);
                    ...
                    if (
                        ...
                        if (arks.includes('-link')) {
                            ...
                            await kill.sendMessage(chatId, { text: baixarYouTube.value }, { quoted: quoteThis });

                            /* Se for áudio */
                        } else if (downFormat === 'audio') {
                            /* Envia como PTT e armazena a ID/PTT */
                            await kill.sendMessage(chatId, { audio: { url: baixarYouTube.value }, mimetype: 'audio/mp4', ptt: true }, { quoted: quoteThis });
                            ...
                            /* Envia como arquivo URL e armazena a ID */
                            await kill.sendMessage(chatId, { video: { url: baixarYouTube.value }, mimetype: 'video/mp4' }, { quoted: quoteThis });
                        }
