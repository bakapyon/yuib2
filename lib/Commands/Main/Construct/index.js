...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            /* Para o lucas do futuro: Append será responsavel por fazer ter novas funções */
            if (!typeof messageData.quoteThis?.message === 'object' || messageData.quoteThis?.message == null || messageData.quoteThis == null || (message.type === 'append' && Object.keys(messageData.quoteThis.message).every((ms) => !ms.includes('Message')))) return postResults(envInfo.results);

            /* Se tiver ephemeralMessage */
            if (Object.keys(messageData?.quoteThis?.message).includes('ephemeralMessage')) {
                ...
                messageData.quoteThis.message = messageData.quoteThis.message.ephemeralMessage;
            }
            ...
            let keysIndex = Object.keys(messageData.quoteThis.message).filter((f) => f !== 'senderKeyDistributionMessage' && f !== 'messageContextInfo');
            let recMessage = messageData.quoteThis.message[keysIndex[0]];

            /* Reorganiza a recMessage se precisar */
            keysIndex = Object.keys(recMessage).filter((f) => f !== 'senderKeyDistributionMessage' && f !== 'messageContextInfo');

            /* Geralmente seria message, mas inseri como index, caso tenha um tipo espefico em mãos */
            recMessage = recMessage?.message ? recMessage?.message[keysIndex[0]] : recMessage;
            ...
            /* Define se deve ignorar as mensagens da Yui, editadas, broadcast e reações */
            if (((chatMessage?.fromMe && config.botCommands.value === false)) || editedMessage || (Object.keys(messageData.quoteThis?.message).includes('pollUpdate') && config.listenPolls.value === false) || (messageData.quoteThis.broadcast === true && config.listenBroadcasts.value === false) || (Object.keys(messageData.quoteThis?.message).includes('reactionMessage') && config.listenReactions.value === false)) {
                /* Define soft error */
            ...
            messageData.userPlatform = chatMessage.id.length > 21 ? 'Android' : messageData.userPlatform;
            ...
            const stckPack = (removeAccents(config.stickerPack.value) || '🔰 Legião Z [linktr.ee/pyon] Yui ⚜️');
            ...
            */
            messageData.stickerConfig = {
                ...
                pack: stckPack,
                type: undefined,
                categories: messageData.body.match(/[\p{Emoji}]/gu) || undefined,
                quality: 100,
            ...
            /* Se for um sticker */
            if (messageData.isSticker || messageData.isQuotedSticker) {
                /* Extrai a metadata */
                messageData.stickerMetadata = await extractMetadata(messageData.decryptedMedia);
            }
