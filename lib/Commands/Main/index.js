...
function endLogger(kill, dataSets, commandExecuter) {
    ...
    if (
        ...
        const extraInfo = ['isGroupAdmins', 'isBlocked', 'isMedia', 'isInvite', 'isOwner', 'isVIP', 'isModerator'].filter((f) => dataSets.value[f] === true);

        /* Realiza a impressão */
        console.log(
            `${Indexer('color').echo(`> ${messageType}`, cmdColor).value}: ${Indexer('color').echo(previewText, msgColor).value}\n`
...
async function redirectCommands(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            /* Para lucas do futuro: Lembre-se de remover isso afim de construir sistemas novos! */
            if (message.type === 'append' && Object.keys(message?.messages[0]?.message).every((ms) => !ms.includes('Message'))) return postResults(envInfo.results);
