    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if ((conditions && type !== 'none') || (conditions2 && type !== 'none')) {
                ...
                if (arks.includes('--help') || type === 'handlers' || args.length === 0) {
                    ...
                    if (['vips', 'mods', 'blacklist', 'prefix', 'whitelist', 'antifake'].includes(type) && argl[0] !== '-text' && !['antilinks', 'level', 'nsfw', 'antispam'].includes(type)) {
                        ...
                        /* Sistemas de texto */
                    } else if (['goodbye', 'promote', 'demote', 'welcome', 'blacklist', 'antifake', 'language'].includes(type) && argl[0] === '-text' && !['antilinks', 'level', 'nsfw', 'antispam'].includes(type)) {
                        /* Define uma RegExp */
                        ...
                        /* Executa a atualização dessa função */
                        Indexer('sql').update('groups', user, chatId, 'antilinks', updateSystem);
                    }
