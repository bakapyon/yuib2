...
if [ "$(ls -A "$sessions_dir")" ]; then
    ...
    printf "[Yui] → Possuo sessões do WhatsApp Web conectadas. Listarei as sessões uma a uma, diga qual quer desconectar.\n"
    ...
    for ((i=0; i<${#session_folders[@]}; i++)); do
        ...
        select opt in "Sim (y)" "Não (n)"; do
            case $opt in
                ...
                # Ignora essa sessão
                "Não (n)")
                    printf "[Yui] → Você optou por não apagar a sessão %s.\n" "$i"
                    break
                ;;
    ...
    printf "[Yui | DONE] → Você ainda não possui sessões conectadas.\n"
fi
