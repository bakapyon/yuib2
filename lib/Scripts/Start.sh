...
# Este script inicia a Yui se os módulos estiverem instalados
...
if [ -d "node_modules" ]; then
    ...
    if [ "$1" == "hide" ]; then
        printf "\n[Yui] → Função iniciada em modo oculto.\n"
    ...
    if [ "$1" == "pm2hide" ]; then
        ...
        if bash ./lib/Scripts/PM2Install.sh; then
            pm2 start ./lib/Initialize/index.js --name yui --cron-restart="0 */6 * * *" > /dev/null 2>&1 &
            printf "\n[Yui] → Função iniciada com PM2 em modo oculto.\n"
            exit 0
        else
            exit 1
        fi
    ...
    if [ "$1" == "pm2show" ]; then
        # Chama o script PM2Install.sh
        if bash ./lib/Scripts/PM2Install.sh; then
            pm2 start ./lib/Initialize/index.js --name yui --cron-restart="0 */6 * * *" && pm2 monit
            printf "\n[Yui] → Função iniciada com PM2 e monitoramento.\n"
            exit 0
        else
            exit 1
        fi
    ...
    if [ "$1" == "autoreboot" ]; then
        while : ; do
            printf "[Yui] → Iniciando Yui, caso o processo sofra falhas, um reinicio automático será feito.\n"
            npm start
            sleep 1
        done
    fi
    ...
    select opt in "Sim (y)" "Não (n)"; do
        case $opt in
            # Chama o script de instalação e, depois que terminar, inicia a Yui
            "Sim (y)")
