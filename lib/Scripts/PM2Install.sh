...
if command -v pm2 &> /dev/null; then
    printf "[Yui] → O comando pm2 já está instalado.\n"
    exit 0
else
    # Se o comando pm2 não está instalado, oferece a opção de instalar
    printf "[Yui] → O comando pm2 não está instalado. Deseja instalá-lo (1 ou 2)?\n\n"
    select opt in "Sim (1)" "Não (2)"; do
        case $opt in
            ...
            "Sim (1)")
                ...
                if  npm i -g pm2; then
                    printf "[Yui | DONE] → Instalação do PM2 concluída com sucesso! <3\n"
                    exit 0
                else
                    printf "[Yui | ERRO] → Ocorreram erros durante a instalação do PM2. Verifique o console e procure o suporte.\n"
                    exit 1
                fi
            ;;
