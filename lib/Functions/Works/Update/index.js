    ...
    try {
        ...
        if (envInfo.parameters.enableChecker.value) {
            ...
            if (
                ...
                console.log(Indexer('color').echo('[UPDATE]', 'red').value, Indexer('color').echo(`${language(region, 'Updates', 'Available', true, true)} → [${packVersion.data.version} ~ ${packVersion.data.build_name.toUpperCase()} ~ ${packVersion.data.build_date.toUpperCase()}] | ${packVersion.data.homepage}`, 'yellow').value);
            }
